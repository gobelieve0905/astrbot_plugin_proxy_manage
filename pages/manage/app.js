...
function render(){
  ...
  if(tab==='overview'){
    ...
    html=`<div class="hero"><div><small>当前配置</small><strong>${esc(state.name)}</strong></div><div class="runtime-state"><span class="${kernelClass}">内核：${kernelText}</span><small>${esc(kernelStatus.message||'')}</small></div></div>
      ...
      <section class="panel"><div class="section-head"><div><small>真实接入范围 · ${esc(state.traffic_audit?.version||'未审计')}</small><h2>AstrBot 流量清单</h2></div></div><div class="traffic-inventory">${(state.traffic_inventory||[]).map(item=>`<article><div><b>${esc(item.name)}</b><small>${esc(item.method)}</small></div><span class="traffic-state ${esc(item.status)}">${esc({managed:'已接管',direct:'明确直连',not_connected:'未接入',unknown:'无法判定'}[item.status]||'无法判定')}</span><p>${esc(item.message)}</p><small>验证：${esc(item.verification||'当前无专项验证')}</small><small>旁路风险：${esc(item.bypass_risk||'待审计')}</small>${item.restart?'<small>变更后需要重启相关组件</small>':''}${item.discovered?.length?`<small>已发现：${esc(item.discovered.map(value=>value.name+' ('+(value.locality_label||value.type||value.transport||'未知')+')').join(' · '))}</small>`:''}</article>`).join('')}</div></section>
    ...
    html=`<section class="panel"><div class="bar"><h2>插件自管内核</h2><div class="actions"><select id="adapter-select">${(state.adapters||[]).map(item=>`<option value="${esc(item.id)}" ${item.id===kernelStatus.adapter||item.id===state.control?.adapter?'selected':''}>${esc(item.id)}</option>`).join('')}</select><button id="adapter-switch">切换内核</button><button id="kernel-install" ${taskRunning?'disabled':''}>下载安装</button>${taskRunning?'<button id="kernel-install-cancel">取消下载</button>':''}<button id="kernel-start">启动</button><button id="kernel-stop">停止</button><button id="runtime-apply" class="primary">应用代理配置</button></div></div><div class="cards"><article><b>${esc(kernelStatus.adapter||state.control?.adapter||'--')}</b><span>当前适配器</span></article><article><b>${esc(artifact.version||'--')}</b><span>固定版本</span></article><article><b>${esc(artifact.platform?.os||'--')} / ${esc(artifact.platform?.arch||'--')}</b><span>${esc(artifact.platform?.libc||'平台')}</span></article><article><b>${esc(process.state||'未知')}</b><span>进程状态</span></article></div>
