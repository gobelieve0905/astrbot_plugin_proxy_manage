...
function render(){
  ...
  if(tab==='overview'){
    ...
    html=`<section class="panel kernel-current"><div class="bar"><div><small>第一栏 · 当前选择 ${esc(currentId||'--')}</small><h2>运行控制</h2></div><div class="actions"><select id="adapter-select" ${runnable.length?'':'disabled'}>${runnable.map(item=>`<option value="${esc(item.id)}" ${item.id===currentId?'selected':''}>${esc(item.display_name||item.id)}</option>`).join('')||'<option>没有可运行内核</option>'}</select><button id="adapter-switch" ${runnable.length?'':'disabled'}>切换内核</button><button id="kernel-start" ${runnable.length?'':'disabled'}>启动</button><button id="kernel-stop">停止</button><button id="control-status">刷新运行状态</button><button id="runtime-apply" class="primary" ${runnable.length?'':'disabled'}>应用代理配置</button></div></div><div class="kernel-platform"><span>当前平台</span><b>${esc(artifact.platform?.os||'--')} / ${esc(artifact.platform?.arch||'--')}</b><small>${esc(artifact.platform?.libc||'无 libc 标识')}</small><span>当前进程</span><b>${esc(process.state||'未知')}</b><small>${esc(kernelStatus.message||'')}</small></div><p class="muted">控制密钥、监听地址和稳定代理入口由插件内部生成并仅绑定回环地址，不需要手工配置。</p></section>
