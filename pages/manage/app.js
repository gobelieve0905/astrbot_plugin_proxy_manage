...
function kernelCard(item,current){
  ...
  const waitingUpload=task.state==='waiting_upload'
  const taskProgress=task.state&&task.state!=='idle'?`<div class="kernel-progress ${waitingUpload?'waiting-upload':''}"><div><b>${esc(task.message||'资源任务处理中')}</b><span>${esc(waitingUpload?'等待离线上传':task.phase||'')}</span></div>${waitingUpload?'<small>请使用下方官方下载地址获取固定版本制品，然后选择文件进行校验并安装。</small>':`<progress max="100" value="${Math.min(100,percent)}"></progress><small>${task.total?formatBytes(task.downloaded||0)+' / '+formatBytes(task.total):percent+'%'}</small>`}</div>`:''
