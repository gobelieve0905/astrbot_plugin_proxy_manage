...
function kernelCard(item,current){
  ...
  return `<article class="kernel-card ${current?'current':''}" data-kernel-card="${esc(item.id)}"><div class="kernel-card-head"><div><small>${current?'当前选择':'资源内核'} · ${item.enabled?'已启用':'未启用'}</small><h3>${esc(item.display_name||item.id)}</h3></div><span class="kernel-badge ${kernelStateClass(state)}">${kernelStateLabel(state)}</span></div><div class="kernel-facts"><div><span>已安装</span><b>${esc(installed)}</b></div><div><span>固定版本</span>${versions?`<select data-kernel-version="${esc(item.id)}">${versions}</select>`:`<b>${esc(version)}</b>`}</div><div><span>平台资源</span><b>${esc(resource.resource_key||artifact.resource_key||'--')}</b></div></div><p class="kernel-message">${esc(task.message||artifact.message||'')}${check.checked_at?`<br>更新检查：${esc(check.message||'')}（${time(check.checked_at)}）`:''}</p>${taskProgress}<div class="kernel-actions">${action}${uninstall}${enable}${select}<button data-kernel-check="${esc(item.id)}">检查更新</button></div>${resource.download_url?`<div class="kernel-download-url"><span>官方下载地址</span><code>${esc(resource.download_url)}</code></div>`:''}<details><summary>制品校验信息</summary><code>${esc(resource.artifact||'资源未配置')}<br>${esc(resource.expected_sha256||'')}</code></details></article>`
}
...
function render(){
  ...
  if(tab==='overview'){
    ...
    html=`<section class="panel kernel-overview"><div class="bar"><div><small>资源管理 · ${installed}/${adapters.length} 已下载 · ${enabled}/${adapters.length} 已启用</small><h2>内核资源管理</h2></div><div class="actions"><button id="refresh-kernels">刷新资源状态</button><button id="check-all-kernels">检查全部更新</button></div></div><p class="muted">插件不会自动下载内核。请先下载并校验需要的内核，再点击“启用”；启用只代表允许运行，不会自动启动。</p><div class="kernel-grid">${adapters.map(item=>kernelCard(item, item.id===currentId&&process.state==='running')).join('')}</div>
      ...
      <section class="panel kernel-current"><div class="bar"><div><small>运行控制 · 当前选择 ${esc(currentId||'--')}</small><h2>运行控制</h2></div><div class="actions"><select id="adapter-select" ${runnable.length?'':'disabled'}>${runnable.map(item=>`<option value="${esc(item.id)}" ${item.id===currentId?'selected':''}>${esc(item.display_name||item.id)}</option>`).join('')||'<option>没有可运行内核</option>'}</select><button id="adapter-switch" ${runnable.length?'':'disabled'}>切换内核</button><button id="kernel-start" ${runnable.length?'':'disabled'}>启动</button><button id="kernel-stop">停止</button><button id="control-status">刷新运行状态</button><button id="runtime-apply" class="primary" ${runnable.length?'':'disabled'}>应用代理配置</button></div></div>
      <div class="offline-install"><b>离线安装</b><div class="inline"><select id="kernel-upload-adapter">${adapters.map(item=>`<option value="${esc(item.id)}" ${item.id===currentId?'selected':''}>${esc(item.display_name||item.id)}</option>`).join('')}</select><select id="kernel-upload-version">${(adapters.find(item=>item.id===currentId)?.artifact?.available_versions||[]).map(value=>`<option value="${esc(value)}">${esc(value)}</option>`).join('')}</select><input id="kernel-file" type="file" accept=".gz,.zip,.tar.gz"><button id="kernel-upload">校验并安装离线制品</button></div></div><p class="muted">控制密钥、监听地址和稳定代理入口由插件内部生成并仅绑定回环地址，不需要手工配置。资源更新不会自动切换未运行的内核；当前内核更新失败会恢复上一份可运行制品。</p></section>
...
function bind(){
  ...
  $('kernel-upload-adapter')?.addEventListener('change',()=>{const item=state.adapters?.find(value=>value.id===$('kernel-upload-adapter').value);$('kernel-upload-version').innerHTML=(item?.artifact?.available_versions||[]).map(value=>`<option value="${esc(value)}">${esc(value)}</option>`).join('')})
...
async function uploadKernel(){const file=$('kernel-file')?.files?.[0],adapter=$('kernel-upload-adapter')?.value,version=$('kernel-upload-version')?.value;if(!file)return note('请选择与当前平台匹配的固定版本制品',true);if(file.size>64*1024*1024)return note('制品超过 64 MiB 限制',true);const item=state.adapters?.find(value=>value.id===adapter);if(item)item.install={state:'running',operation:'install',phase:'uploading',progress:10,message:'正在上传离线制品'};render();try{note('正在校验离线制品...');const content=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result).split(',',2)[1]);reader.onerror=reject;reader.readAsDataURL(file)});await api.apiPost('kernel-upload',{adapter,version,content});await load();note('离线制品已校验并安装；请在资源卡片中启用')}catch(error){await load();note(error.message,true)}}
