...
function render(){
  ...
  if(tab==='overview'){
    ...
    html=`<div class="hero"><div><small>当前配置</small><strong>${esc(state.name)}</strong></div><div class="runtime-state"><span class="${kernelClass}">内核：${kernelText}</span><small>${esc(kernelStatus.message||'')}</small></div></div>
      ...
      <section class="panel"><div class="section-head"><div><small>真实接入范围</small><h2>AstrBot 流量清单</h2></div></div><div class="traffic-inventory">${(state.traffic_inventory||[]).map(item=>`<article><div><b>${esc(item.name)}</b><small>${esc(item.method)}</small></div><span class="traffic-state ${esc(item.status)}">${esc({managed:'已接管',direct:'明确直连',not_connected:'未接入',unknown:'无法判定'}[item.status]||'无法判定')}</span><p>${esc(item.message)}</p>${item.restart?'<small>变更后需要重启相关组件</small>':''}</article>`).join('')}</div></section>
