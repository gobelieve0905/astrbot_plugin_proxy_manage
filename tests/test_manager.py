...
class TestConfigurationRules(unittest.TestCase):
    ...
    def test_management_page_has_responsive_navigation_and_stable_controls(self):
        ...
        self.assertIn('group-accordion',script)
        self.assertIn('data-group-accordion',script)
        self.assertIn('group-runtime-inline',script)
        self.assertIn('data-group-id',script)
        self.assertNotIn('groups-runtime-section',script)
        self.assertIn('proxy-node-dialog',html)
        ...
        self.assertIn('id="rule-group-dialog"',html)
        self.assertIn('id="action-confirm-dialog"',html)
        self.assertIn('id="subscription-detail-dialog"',html)
        self.assertIn('id="audit-detail-dialog"',html)
        self.assertIn('data-edit-rule',script)
        self.assertIn('data-audit-details',script)
        self.assertIn('function openAuditDialog(index)',script)
        self.assertIn('function openSubscriptionDialog(id)',script)
        self.assertIn('data-sub-details',script)
        self.assertNotIn('<input data-k="name"',script)
        self.assertNotIn('<input class="interval"',script)
        self.assertIn('function orderedRuleGroups()',script)
        self.assertIn('function moveRuleGroup(sourceId,targetId,after=false)',script)
        self.assertIn('data-rule-drag-handle',script)
        self.assertIn('data-rule-move="up"',script)
        self.assertIn('data-rule-target',script)
        self.assertIn('orderedRules.map((rule,index)=>',script)
        self.assertIn('rule.priority=index+1',script)
        self.assertNotIn('rule-priority',script[script.index('function renderRuleDialog()'):script.index('function openRuleDialog(')])
        self.assertIn('id="rule-template"',script)
        self.assertIn('function templateDomains(template)',script)
        self.assertNotIn('data-template-use',script)
        self.assertNotIn("openRuleDialog('',button.dataset.template)",script)
        self.assertIn('data-rule-domain-match',script)
        self.assertIn('data-rule-domain-host',script)
        self.assertIn('id="rule-domain-add"',script)
        ...
        self.assertIn('showPopover()',script)
        self.assertIn('if(!state.rule_groups.length)state.routes=[]',script)
        self.assertNotIn('confirm(',script)
        self.assertIn('save-only',html)
        self.assertIn('async function applyConfiguration()',script)
        node_view=script[script.index("} else if(tab==='nodes')"):script.index("} else if(tab==='groups')")]
        self.assertNotIn('<input data-k="name"',node_view)
        self.assertIn('data-node-field="name"',script)
