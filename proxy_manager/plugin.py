...
class ProxyManager(Star):
        ...
        try:
            ...
            try:
                async with httpx.AsyncClient(**client_options) as client:
                    response=await client.get(url,headers={'User-Agent':'astrbot-proxy-route-verifier/0.3.18'})
            finally:
    ...
    async def subscription_preview(self):
        try:
            ...
            for index,entry in enumerate(entries):
                ...
                response=await fetch_public_url(url,headers={'User-Agent':'astrbot-plugin-proxy-manage/0.3.18'})
    ...
    async def _refresh_subscription(self,subscription_id:str):
        async with self.refresh_lock:
            ...
            response=await fetch_public_url(subscription['url'],headers={'User-Agent':'astrbot-plugin-proxy-manage/0.3.18'})
            if response.status_code>=400 or len(response.content)>10*1024*1024:
