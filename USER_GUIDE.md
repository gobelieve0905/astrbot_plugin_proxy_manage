...
1. 打开“内核管理”的“内核资源管理”，查看 Xray、Mihomo 和 sing-box 资源卡片。插件不会自动下载；按当前平台选择固定制品，点击“下载安装”，校验完成后再点击“启用”。页面按当前平台显示 SHA-256、已安装版本、固定版本和更新检查结果；`未安装`、`平台不支持`、`校验失败` 和 `有更新` 均为独立状态。
2. 全部受信来源超时或失败时，页面会进入“等待离线上传”。使用页面展示的官方地址在本地下载对应制品，核对文件名后上传；插件仍会验证固定 SHA-256。
3. 若固定清单存在多个版本，可在资源卡片中选择目标版本。点击“检查更新”只查询官方稳定版本，不会自动下载；只有已进入固定清单并有 SHA-256 的版本才能安装。更新当前运行内核时会受控重启，启动或健康检查失败时自动恢复上一份可运行制品。控制地址、Secret、监听地址和代理入口均由插件内部管理，无需手工填写。
